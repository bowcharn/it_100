...
//从尾到头打印出链表的每个元素
void print_list_reversely(node* list_head)
{
	if(list_head)
	{
		print_list_reversely(list_head->next);
		std::cout<<list_head->data<<" ";
	}
}
...
int main()
{
  ...
  create_list(&a_list,a1, 5);
  create_list(&b_list,a2,5);

  std::cout<<"print in order...\n";
  print_list(a_list);
  std::cout<<"print reversely...\n";
  print_list_reversely(a_list);
