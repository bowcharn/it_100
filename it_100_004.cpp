...
/*
  2014.3.14
  关于二叉树和BST（Binary Search Tree）的基本操作和常见问题解决， 目前主要应用了递归：
  非递归方式暂时没有给出
...
注对于该函数的实现函数原型为：
void print_all_paths_of_sum(tree_node* root, 
							int sum ,
							std::vector<int> path_to_print)
							...
							std::vector<int> path_to_print)
{
	...
	path_to_print.push_back(root->data);
	if(root->data == sum && root->right == NULL && root->left == NULL)
	{
		std::vector<int>::iterator itr;
		for(itr = path_to_print.begin(); itr != path_to_print.end(); itr++)
		{
			if(itr == path_to_print.end()-1)
	...
	print_all_paths_of_sum(root->left, sum - root->data, path_to_print);
	print_all_paths_of_sum(root->right, sum - root->data, path_to_print);
...
int main()
{
  ...
  create_tree(tree4, ary4, 5);

  //print_paths(tree4);
  std::vector<int> path_to_print;
  print_all_paths_of_sum(tree4, 22, path_to_print);
