/*
  2014.3.11
  该文件编写了基本的单链表操作，其中很多函数还是有一定困难的，后期在使用链表的
  时候可以参看此cpp的思想；
  
  
  
  题目：输入一个单向链表，输出该链表中倒数第k个结点。链表的倒数第0个结点为
  链表的尾指针。链表结点定义如下：
struct ListNode
{
      int       m_nKey;
...
实现该程序为本文件的目的，函数原型为：
node* find_kth_to_tail(node* list_head, int k)
*/
...
node* find_kth_to_tail(node* list_head, int k)
{
	...
	node* pfirst = list_head, *psecond = list_head;
	int cnt = 0;
	while(cnt++ != k-1)
		pfirst = pfirst->next;
...
int main()
{
  int a1[10] = {1,2,3,5,7,4,5,34};
  int a2[10] = {2,5,7,9,10};
  node* a_list = NULL;
  node* b_list = NULL;
 
  create_list(&a_list,a1, 7);
  create_list(&b_list,a2,5);

  std::cout<<"-4th:: "<< find_kth_to_tail(a_list, 4) -> data<<std::endl;
